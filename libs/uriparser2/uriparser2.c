...
static void parse_user_info(URI *uri, const UriTextRangeA *r, char **buffer) {
	...
	if (size) {
		...
		if (user_size) {
			uri->user_ = memcpyz(*buffer, r->first, user_size);
			*buffer += user_size + 1;
		}
		if (pass_size) {
			uri->pass_ = memcpyz(*buffer, colon + 1, pass_size);
			*buffer += pass_size + 1;
		}
	}
}
...
static void init_uri(const UriUriA *uu, URI *uri, char *buffer) {
	uri->scheme_ = copy_range(&uu->scheme, &buffer);
	uri->user_ = 0;
	uri->pass_ = 0;
	uri->host_ = copy_range(&uu->hostText, &buffer);
	uri->port_ = parse_int(uu->portText.first, uu->portText.afterLast);
	uri->path_ = copy_path(uu->pathHead, &buffer);
	uri->query_ = copy_range(&uu->query, &buffer);
	uri->fragment_ = copy_range(&uu->fragment, &buffer);
...
char *uri_build(const URI *uri) {
	...
	if (uri->scheme_) {
		size += strlen(uri->scheme_) + 3;	/* "://" */
	}
	if (uri->user_) {
		size += strlen(uri->user_) + 1;		/* ":" or "@" */
	}
	if (uri->pass_) {
		size += strlen(uri->pass_) + 1;		/* "@" */
	}
	if (uri->host_) {
		size += strlen(uri->host_);
	}
	if (uri->port_) {
		size += 1 + power_of_10(uri->port_);	/* ":" port */
	}
	if (uri->path_) {
		size += strlen(uri->path_);
	}
	if (uri->query_) {
		size += 1 + strlen(uri->query_);		/* "?" query */
	}
	if (uri->fragment_) {
		size += 1 + strlen(uri->fragment_);	/* "#" fragment */
	}
	...
	if (s) {
		char *p = s;
		if (uri->scheme_) {
			p = append(p, uri->scheme_);
			*p++ = ':';
			*p++ = '/';
			*p++ = '/';
		}
		if (uri->user_) {
			p = append(p, uri->user_);
		}
		if (uri->pass_) {
			*p++ = ':';
			p = append(p, uri->pass_);
		}
		if (uri->user_ || uri->pass_) {
			*p++ = '@';
		}
		if (uri->host_) {
			p = append(p, uri->host_);
		}
		if (uri->port_) {
			p += sprintf(p, ":%d", uri->port_);
		}
		if (uri->path_) {
			p = append(p, uri->path_);
		}
		if (uri->query_) {
			*p++ = '?';
			p = append(p, uri->query_);
		}
		if (uri->fragment_) {
			*p++ = '#';
			p = append(p, uri->fragment_);
		}
		*p = '\0';
	}
...
int uri_compare(const URI *a, const URI *b) {
	COMPARE(a->scheme_, b->scheme_);
	COMPARE(a->host_, b->host_);

	if (a->port_ != b->port_) {
		return a->port_ > b->port_ ? 1 : -1;
	}

	COMPARE(a->path_, b->path_);
	COMPARE(a->query_, b->query_);
	COMPARE(a->fragment_, b->fragment_);

	COMPARE(a->user_, b->user_);
	COMPARE(a->pass_, b->pass_);
