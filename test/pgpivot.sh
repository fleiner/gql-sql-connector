...
$srcdir/postgresqlcheck.py \
    ...
    -c "Sales%2C12:00:00.000 sum(salary),Sales%2C12:00:00.000 sum(salary),number,General" \
     -v -d pv -- \
    'select sum(salary) pivot dept, lunchTime order by dept, lunchTime' \
...
$srcdir/postgresqlcheck.py \
    ...
    -v -d pv -- \
    'select sum(salary) pivot dept order by dept format sum(salary) "#,##"' \
...
$srcdir/postgresqlcheck.py \
    ...
    -v -d pv -- \
    'select dept, sum(salary) group by dept pivot lunchTime order by dept format sum(salary) "#,###"' \
...
$srcdir/postgresqlcheck.py \
    ...
    -v -d pv -- \
    'select lunchTime, sum(salary) group by lunchTime pivot dept order by dept' \
...
$srcdir/postgresqlcheck.py \
    ...
    -v -d pv -- \
        'select sum(salary), max(lunchTime) pivot dept order by dept label sum(salary) "PAY"' \
